...
- **Fixed:** Loader / persistent snackbar freeze — same root cause as above. `dismissById(loadId)` called after `await future` hit the no-op placeholder and the loading indicator never disappeared, blocking all subsequent notifications.
- **Fixed:** `dismissAll()` and `dismissGroup()` now correctly target every active entry in `_activeEntries` instead of only `_current`, so all visible notifications are dismissed as expected.
- **Fixed:** Progress bar duration not respected — the bar was driven by `_progressCtrl.forward()` (0.0 → 1.0), which filled the bar rather than depleting it, and used `reverseDuration` inconsistently. Changed to `_progressCtrl.value = 1.0` then `_progressCtrl.reverse()` with `reverseDuration = opts.duration`, so the bar accurately depletes from full to empty over exactly the notification's `duration`.
- **Fixed:** `Navigator.of(context).pushNamed()` inside `_NotificationCard` threw *"Navigator operation requested with a context that does not include a Navigator"* because the card lives in an `Overlay` with no Navigator ancestor. Changed to `AwesomeSnackbar.navigatorKey.currentState?.pushNamed()`.
- **Fixed:** `AwesomeHaptic.vibrate` was incorrectly mapped to `HapticFeedback.heavyImpact()`. Now correctly calls `HapticFeedback.vibrate()`.
- **Fixed:** `registerActive()` was called after `_ctrl.forward()`, creating a narrow race window where a `dismissById()` arriving during the entrance animation would hit the no-op placeholder. Moved `registerActive()` to before `_ctrl.forward()` so the real callback is always in place from the moment the widget mounts.
- **Fixed:** `_dismiss()` had a missing `onDone()` call on the post-`await` unmounted path — if the widget was disposed between `markExiting()` and the completion of `_ctrl.reverse()` (e.g. hot reload, route pop), `unregisterActive()` was never called, permanently blocking the queue. Added a consistent `onDone()` call on every exit path.
- **Fixed:** `_isExiting` flag was never cleared when multiple entries were in flight — `unregisterActive()` now checks whether any remaining active entry is still exiting before clearing the flag, preventing premature queue promotion.
